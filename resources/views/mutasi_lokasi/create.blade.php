...
<div class="edit-card">
    <div class="edit-header">
        <h5 class="mb-0">
            <i class="fas fa-plus-circle me-2"></i>Tambah Mutasi Lokasi
        </h5>
        <p class="mb-0 text-white-50">Tambah data mutasi lokasi baru</p>
    </div>
                        ...
                        <select class="form-control @error('id_pengadaan') is-invalid @enderror" 
                            ...
                            @foreach($pengadaans as $pengadaan)
                                <option value="{{ $pengadaan->id_pengadaan }}" 
                                    {{ old('id_pengadaan') == $pengadaan->id_pengadaan ? 'selected' : '' }}>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('flag_lokasi') is-invalid @enderror"
                               id="flag_lokasi" name="flag_lokasi" value="{{ old('flag_lokasi') }}" required>
                        @error('flag_lokasi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('flag_pindah') is-invalid @enderror"
                               id="flag_pindah" name="flag_pindah" value="{{ old('flag_pindah') }}" required>
                        @error('flag_pindah')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
